...
static float *rbin, *deltar, *Deltar, *zetar;
static int nr;
...
double sinc(double x) {
  ...
  else return 1. - ipow(x,2)/6. + ipow(x,4)/120. - ipow(x,6)/5040. + ipow(x,8)/362880.;
}
...
void get_delta() {
  ptrdiff_t i,p;
  for(i=0;i<nr;i++) {
    do_c2r_fun(&sinc,rbin[i]);
    for(p=0;p<npeak;p++) deltar[i+nr*p] = delta[kh[p]+npad*(jh[p]+n*(ih[p]-local_0_start))];
    if(!my_id) printf("  computing delta(%g)\n",rbin[i]);
  }
}
void get_Delta() {
  ptrdiff_t i,p;
  for(i=0;i<nr;i++) {
    do_c2r_fun(&tophat,rbin[i]);
    for(p=0;p<npeak;p++) Deltar[i+nr*p] = delta[kh[p]+npad*(jh[p]+n*(ih[p]-local_0_start))];
    if(!my_id) printf("  computing Delta(%g)\n",rbin[i]);
  }
}
void get_zeta() {
  ptrdiff_t i,p;
  for(i=0;i<nr;i++) {
    do_c2r_fun(&cos,rbin[i]);
    for(p=0;p<npeak;p++) zetar[i+nr*p] = delta[kh[p]+npad*(jh[p]+n*(ih[p]-local_0_start))];
    if(!my_id) printf("  computing zeta(%g)\n",rbin[i]);
  }
}
...
int main(int argc, char **argv) {
  ...
  deltar = (float *)malloc(sizeof(float)*npeak*nr);
  Deltar = (float *)malloc(sizeof(float)*npeak*nr);
  zetar = (float *)malloc(sizeof(float)*npeak*nr);
  ...
  prepare_r(1./n,.25,1);
  get_delta();
  if(!my_id) printf("sinc profile done (t=%.0lfs)\n",difftime(time(NULL),timer));
  get_Delta();
  if(!my_id) printf("W profile done (t=%.0lfs)\n",difftime(time(NULL),timer));
  get_zeta();
  if(!my_id) printf("cos profile done (t=%.0lfs)\n",difftime(time(NULL),timer));

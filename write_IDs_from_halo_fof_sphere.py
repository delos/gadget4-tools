...
def sphere(ID, GridSize):
  ...
  X, Y, Z = (X[mask]+xc[0])%GridSize, (Y[mask]+xc[1])%GridSize, (Z[mask]+xc[2])%GridSize
...
def run(argv):
  ...
  GridSize = int(argv[3])
  ID_enclosing = sphere(ID,GridSize)

  print('%d particles in Lagrangian sphere'%(ID_enclosing.size))

  if not np.all(np.isin(ID,ID_enclosing)):
    raise Exception('halo is not entirely in Lagrangian sphere')

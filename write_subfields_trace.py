...
def run(argv):
  ...
  if len(argv) < 5:
    print('python script.py <snapshot min,max> <grid size> <trace-file> <r> [out-name]')
    return 1
  ...
  for i, filename in enumerate(names):
    ...
    if snapshot_number < _ss[0]:
      x,y,z = _x[0],_y[0],_z[0]
    elif snapshot_number > _ss[-1]:
      x,y,z = _x[-1],_y[-1],_z[-1]
    else:
      x = _x[_ss==snapshot_number][0]
      y = _y[_ss==snapshot_number][0]
      z = _z[_ss==snapshot_number][0]

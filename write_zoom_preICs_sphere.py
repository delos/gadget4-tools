...
@njit
def IDs_of_sphere(ctr,r,GridSize):
  out = np.zeros(int(8*r**3),dtype=np.uint32)
  i = 0
  for cell in range(GridSize**3):
    x = cell // GridSize**2 - ctr[0];
    xr = cell % GridSize**2;
    y = xr // GridSize - ctr[1];
    z = xr % GridSize - ctr[2];
    if x >= GridSize//2:
    ...
    if z >= GridSize//2:
      z -= GridSize
    if x**2 + y**2 + z**2 <= r**2:
      out[i] = cell+1 # add 1, gadget convention
      i += 1
  return out[:i]
...
def run(argv):
  ...
  if len(argv) < 4:
    print('python script.py <x,y,z,r (grid units)> <glass-file> <any snapshot> [out-prefix]')
    return 1
  ...
  ctr = np.array([float(x) for x in argv[1].split(',')[:3]])
  rad = float(argv[1].split(',')[3])
  print('center = %.0f,%.0f,%.0f'%tuple(ctr) + ', radius = %.0f'%rad)
  IDh = IDs_of_sphere(ctr,rad,GridSize)
  IDh.sort()
  IDh -= 1 # start from 0
